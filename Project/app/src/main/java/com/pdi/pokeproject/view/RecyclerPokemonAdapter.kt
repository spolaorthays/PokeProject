...
class RecyclerPokemonAdapter: RecyclerView.Adapter<RecyclerPokemonAdapter.PokeHolder>() {
    ...
    inner class PokeHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        private var forma1 = view.findViewById<TextView>(R.id.pokemon_forma) //TODO forma é uma lista, então terei que receber uma lista aqui também

        fun bind(pokemon: Pokemon) {
            name.text = pokemon.name
            url.text = pokemon.url
            Picasso.get().load(pokemon.pokemonDetails.sprites.other.officialArtwotk.frontDefault).into(image) //TODO Tanto esse quanto a forma fica repetido
            forma1.text = pokemon.pokemonDetails.forms[0].name
        }
    }
}